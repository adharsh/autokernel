...
_agent_start_prompt() {
  ...
  cat <<EOF
You are agent $agent_id. AGENT_ID=$agent_id, WORKTREE_PATH=$worktree, CUDA_VISIBLE_DEVICES=$gpu_id, AUTOKERNEL_ROOT=$ROOT, AUTOKERNEL_EXPERIMENTS_TSV=$TSV, AUTOKERNEL_EXPERIMENTS_DIR=$EXPERIMENTS_DIR, AUTOKERNEL_REFERENCE_TIMING_PATH=$REFERENCE_TIMING_PATH. Read and follow instructions.md in $ROOT, then read every file under hints/ before choosing the first hypothesis. Required: submit honest general implementations; do not memoize answers, hardcode outputs, special-case tests/benchmarks, detect evaluator behavior, skip correctness paths, or reward-hack. This is training forward, not inference prepack: do not add FP8-packed weights, precomputed scales, expanded scale metadata, activation scale state, warmup caches, or hardcoded activation scales as official inputs. Under the current benchmark contract, compute FP8 conversion/scale work inside the measured candidate invocation; only a human-approved contract change can make external FP8/scale state official. Treat hint-defined invalid, stale, already-covered, or diagnostic-only implementation families as scratch: do not run, profile, record, promote, set current_base from them, or set best_speedup from them. Use finalized keep rows in the shared TSV, not pending notes or partial profiles, as the speedup baseline for keep/discard decisions. When validate.py prints diagnostic_quality or candidate_diagnostics, use those rows in ranking and keep/discard decisions, not just in prose. Run scripts/profile_ncu.sh EXPERIMENT_ID basic for every baseline and every recordable experiment that launches kernels; use detailed/full supplemental profiles only when instructions say deeper evidence is needed. After each profile, you MUST read the complete relevant details file from top to bottom (ncu/details.txt, ncu/detailed/details.txt, or ncu/full/details.txt); targeted greps/summaries are allowed only after the full read, not instead of it. The profiler's warnings and recommendations are evidence: consider them, then either act on them or explicitly discard them with a reason in note.md. After each profile, create or update note.md with the measured bottleneck, profile evidence, speed-of-light interpretation when relevant, profiler recommendations considered, and next experiment chosen from that evidence; before recording, finalize that same note. Do not move on without a complete note for every recorded experiment. Record whether PTX/SASS/codegen was inspected. If no obvious speedup is available, escalate profiling only as justified by instructions, then try lower-level optimizations including CUDA C++ with inline PTX when appropriate. Start the experiment loop now. Never stop. Never ask the user anything.
EOF
}
