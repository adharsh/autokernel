...
cmd_start() {
  ...
  for GPU_ID in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    _launch_agent "$GPU_ID" "$AGENT_ID" "$WORKTREE" "$LOG" \
      "You are agent $AGENT_ID. AGENT_ID=$AGENT_ID, WORKTREE_PATH=$WORKTREE, CUDA_VISIBLE_DEVICES=$GPU_ID, AUTOKERNEL_ROOT=$ROOT, AUTOKERNEL_EXPERIMENTS_TSV=$TSV, AUTOKERNEL_EXPERIMENTS_DIR=$EXPERIMENTS_DIR, AUTOKERNEL_REFERENCE_TIMING_PATH=$REFERENCE_TIMING_PATH. Read and follow instructions.md in $ROOT. Required: submit honest general implementations; do not memoize answers, hardcode outputs, special-case tests/benchmarks, detect evaluator behavior, skip correctness paths, or reward-hack. Run scripts/profile_ncu.sh for every baseline and every experiment that launches kernels. After each profile, write note.md with the measured bottleneck, profile evidence, speed-of-light interpretation when relevant, and next experiment chosen from that evidence. Do not move on with a shallow note. Record whether PTX/SASS/codegen was inspected. If no obvious speedup is available, inspect the full profiling details and try justified lower-level optimizations, including CUDA C++ with inline PTX when appropriate. Start the experiment loop now. Never stop. Never ask the user anything."
