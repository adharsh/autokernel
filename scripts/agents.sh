...
CODEX_COMMON_ARGS=(
  ...
  -c "features.fast_mode=$CODEX_FAST_MODE"
)
...
_agent_model_summary() {
  case "$AGENT_CLI" in
    ...
    codex)  printf "model=%s effort=%s service_tier=%s fast_mode=%s\n" \
      "$CODEX_MODEL" "$CODEX_EFFORT" "$CODEX_SERVICE_TIER" "$CODEX_FAST_MODE" ;;
  esac
}
